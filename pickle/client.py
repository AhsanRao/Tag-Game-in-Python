...
class Player():
    ...
    def __init__(self, connection):
        ...
        packet_size = connection.player_socket.recv(connection.header_length)
        player_info_pickle = connection.player_socket.recv(int(packet_size))
...
class Game():
        ...
        def send_player_info(self):
            ...
            while len(header) < self.connection.header_length:
                header += " "  
            self.connection.player_socket.sendall(header.encode())
            self.connection.player_socket.sendall(player_info_pickle)
        ...
        def recieve_player_info(self):
            ...
            player_info_pickle = self.connection.player_socket.recv(int(packet_size))
            player_info = pickle.loads(player_info_pickle)
        ...
        def recieve_pregame_state(self):
            ...
            while self.player_count < self.total_players:
                ...
                packet_size = self.connection.player_socket.recv(self.connection.header_length).decode(self.connection.encoder)
                game_state_pickle = self.connection.player_socket.recv(int(packet_size))
                game_state = pickle.loads(game_state_pickle)
